...
export default tseslint.config(
  ...
  // These supplement strictTypeChecked, which does not include them.
  {
    rules: {
      ...
      // Since typescript-eslint 8.69 this rule also reports `void` on
      // non-call expressions (typescript-eslint PR #12727). This codebase
      // uses `void expr;` for reads whose side effect is the point: layout
      // reads that force reflow and reactive-dependency reads in $effect.
      // no-unused-expressions rejects the bare read, so the pair leaves no
      // compliant spelling for an intentional read. Disable this rule and
      // keep no-unused-expressions.
