...
const en_demo_narrative_topic_list_stats_body = /** @type {(inputs: Demo_Narrative_Topic_List_Stats_BodyInputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`The row under the page title shows live counts for new, active, and on hold tickets, as well as a new replies count once the browser finishes checking read state.
...
const es_demo_narrative_topic_list_stats_body = /** @type {(inputs: Demo_Narrative_Topic_List_Stats_BodyInputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`La fila debajo del título de la página muestra conteos en tiempo real de tickets nuevos, activos y en espera, así como un conteo de nuevas respuestas una vez que el navegador termina de comprobar el estado de lectura.
