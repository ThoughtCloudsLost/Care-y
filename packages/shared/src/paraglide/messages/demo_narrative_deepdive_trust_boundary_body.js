...
- The client's name, alias, phone number and email address. [[#trust-boundary #client-data]]
...
- On \`clients\`, \`encrypted_alias\` is bytea with an \`alias_hash\` blind index beside it, and the phone reference is a foreign key to \`phones\`, where \`encrypted_number\` is bytea beside a \`phone_hash\` blind index. [[#server-holds #client-data]]
...
Row counts, foreign key relationships and timestamp sequences are readable structurally throughout, which is what makes the activity inference above possible. [[#metadata #server-holds]]
**Audit and operational records.** The audit log is pseudonymous and largely plaintext by design, holding an event type, an actor id, an optional case id and a JSON metadata object, so an administrator's browser can query it without a decryption pass. It carries no case content, no phone numbers and no client aliases. Session records, rate-limit counters, job queue rows and the key-evaluation audit log are operational-tier data the server reads directly. [[#server-holds #metadata #privacy]]
...
- El nombre, el alias, el número de teléfono y la dirección de correo del cliente. [[#trust-boundary #client-data]]
...
Terceras partes tienen sus propias copias de algunas cosas, y CARE-Y no puede cifrar lo que nunca tocó primero. Un mensaje de texto existió en la red de la compañía telefónica y en el proveedor de telefonía antes de llegar a CARE-Y. Un correo existió en el propio sistema de correo de quien lo envió. [El relay de telefonía](#deep-dive/the-telephony-relay) cubre qué hace CARE-Y al respecto, y [El ciclo de vida del canal del portal](#deep-dive/portal-channel-lifecycle) cubre los canales que evitan a terceras partes por completo. [[#telephony #portal #trust-boundary]]
...
- En \`clients\`, \`encrypted_alias\` es bytea con un índice ciego \`alias_hash\` al lado, y la referencia al teléfono es una clave externa a \`phones\`, donde \`encrypted_number\` es bytea junto a un índice ciego \`phone_hash\`. [[#server-holds #client-data]]
...
El número de filas, las relaciones de claves externas y las secuencias de marcas de tiempo son legibles estructuralmente en todo el sistema, que es lo que hace posible la inferencia de actividad anterior. [[#metadata #server-holds]]
**Registros de auditoría y operativos.** El registro de auditoría es seudónimo y en gran parte texto plano por diseño, y contiene un tipo de evento, un identificador de actor, un identificador de caso opcional y un objeto JSON de metadatos, de modo que el navegador de una persona administradora puede consultarlo sin una pasada de descifrado. No lleva contenido de casos, ni números de teléfono, ni alias de clientes. Los registros de sesión, los contadores de límite de tasa, las filas de la cola de trabajos y el registro de auditoría de evaluación de claves son datos del nivel operativo que el servidor lee directamente. [[#server-holds #metadata #privacy]]
