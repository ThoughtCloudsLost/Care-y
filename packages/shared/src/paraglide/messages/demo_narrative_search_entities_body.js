...
const en_demo_narrative_search_entities_body = /** @type {(inputs: Demo_Narrative_Search_Entities_BodyInputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`**Tickets.** Matches on decrypted title, client alias, queue name, and assignee name. Full deep search also matches on message content within tickets.
...
const es_demo_narrative_search_entities_body = /** @type {(inputs: Demo_Narrative_Search_Entities_BodyInputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`**Tickets.** Coincide con título descifrado, alias del cliente, nombre de la cola y nombre del asignado. La búsqueda profunda completa también coincide con el contenido de los mensajes dentro de los tickets.
