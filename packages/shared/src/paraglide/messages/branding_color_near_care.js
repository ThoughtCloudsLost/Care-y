...
const en_branding_color_near_care = /** @type {(inputs: Branding_Color_Near_CareInputs) => LocalizedString} */ (i) => {
	return /** @type {LocalizedString} */ (`This shade sits close to the ochre ${i?.volunteers} see on high-priority ${i?.tickets}. A nudged shade keeps your identity and their signal apart.`)
};

const es_branding_color_near_care = /** @type {(inputs: Branding_Color_Near_CareInputs) => LocalizedString} */ (i) => {
	return /** @type {LocalizedString} */ (`Este tono queda cerca del ocre que ven ${i?.volunteers} en ${i?.tickets} de prioridad alta. Un tono ajustado mantiene tu identidad y esa señal separadas.`)
};
