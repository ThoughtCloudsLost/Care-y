...
const en_demo_role_admin_tooltip = /** @type {(inputs: Demo_Role_Admin_TooltipInputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Admin user: full access to all features. Switch users to see how server-side enforcement changes every screen.`)
};

const es_demo_role_admin_tooltip = /** @type {(inputs: Demo_Role_Admin_TooltipInputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Usuario administrador: acceso completo a todas las funciones. Cambia de usuario para ver como la autorizacion del servidor cambia cada pantalla.`)
};
