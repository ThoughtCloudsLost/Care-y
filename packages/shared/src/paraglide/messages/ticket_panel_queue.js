...
const en_xa2_ticket_panel_queue = /** @type {(inputs: Ticket_Panel_QueueInputs) => LocalizedString} */ (i) => {
	return /** @type {LocalizedString} */ (`⟦${i?.Queue}⟧`)
};
...
*/
export const ticket_panel_queue = /** @type {((inputs: Ticket_Panel_QueueInputs, options?: { locale?: "en" | "es" | "en-XA" }) => LocalizedString) & import('../runtime.js').MessageMetadata<Ticket_Panel_QueueInputs, { locale?: "en" | "es" | "en-XA" }, {}>} */ ((inputs, options = {}) => {
	const locale = experimentalStaticLocale ?? options.locale ?? getLocale()
	if (locale === "es") return es_ticket_panel_queue(inputs)
	if (locale === "en-XA") return en_xa2_ticket_panel_queue(inputs)
