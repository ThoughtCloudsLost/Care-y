...
const en_demo_section_admin_comms_desc = /** @type {(inputs: Demo_Section_Admin_Comms_DescInputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`The communications page configures how the organization talks to clients by phone, text, and email. It covers the channel policy that controls which channels are available to volunteers, phone lines and their greetings, SMS templates, the blocklist, and the voicemail quarantine where recordings from unknown callers wait for review. A row of section buttons under the title jumps to any of them.`)
};

const es_demo_section_admin_comms_desc = /** @type {(inputs: Demo_Section_Admin_Comms_DescInputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`La página de comunicaciones configura cómo la organización se comunica con los clientes por teléfono, mensajes de texto y correo electrónico. Cubre la política de canales que controla qué canales están disponibles para los voluntarios, las líneas telefónicas y sus saludos, las plantillas de SMS, la lista de bloqueo y la cuarentena de mensajes de voz donde las grabaciones de llamantes desconocidos esperan revisión. Una fila de botones de sección debajo del título salta a cualquiera de ellos.`)
};

/**
* | output |
* | --- |
* | "The communications page configures how the organization talks to clients by phone, text, and email. It covers the channel policy that controls which channels..." |
*
