...
const en_demo_section_tickets_desc = /** @type {(inputs: Demo_Section_Tickets_DescInputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`The ticket list is where volunteers manage incoming cases. Titles, descriptions, and every message are encrypted with keys only the browser holds. The server stores ciphertext and routes it without reading it.`)
};

const es_demo_section_tickets_desc = /** @type {(inputs: Demo_Section_Tickets_DescInputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`La lista de tickets es donde los voluntarios gestionan los casos entrantes. Los titulos, descripciones y cada mensaje se cifran con claves que solo el navegador posee. El servidor almacena texto cifrado y lo transmite sin leerlo.`)
};
