...
const en_xa2_intake_responses_submitted_at = /** @type {(inputs: Intake_Responses_Submitted_AtInputs) => LocalizedString} */ (i) => {
	return /** @type {LocalizedString} */ (`⟦Sùbmìttèd  •••${i?.date}⟧`)
};
...
*/
export const intake_responses_submitted_at = /** @type {((inputs: Intake_Responses_Submitted_AtInputs, options?: { locale?: "en" | "es" | "en-XA" }) => LocalizedString) & import('../runtime.js').MessageMetadata<Intake_Responses_Submitted_AtInputs, { locale?: "en" | "es" | "en-XA" }, {}>} */ ((inputs, options = {}) => {
	const locale = experimentalStaticLocale ?? options.locale ?? getLocale()
	if (locale === "es") return es_intake_responses_submitted_at(inputs)
	if (locale === "en-XA") return en_xa2_intake_responses_submitted_at(inputs)
