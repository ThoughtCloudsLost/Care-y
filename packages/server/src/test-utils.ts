...
import type { RistrettoPoint, Scalar } from "@care-y/crypto";
...
/**
 * Real ristretto keypair for tests that must seal to a public key and
 * decrypt with the private half. Dummy 0xbb buffers are not valid
 * ristretto points and make eciesEncrypt throw.
 */
export function makeRistrettoKeypair(): {
  priv: Scalar;
  pub: RistrettoPoint;
} {
  const wasmSodium = requireSodium();
  const priv = wasmSodium.crypto_core_ristretto255_scalar_random() as Scalar;
  const pub = wasmSodium.crypto_scalarmult_ristretto255_base(
    priv,
  ) as RistrettoPoint;
  return { priv, pub };
}
...
/**
 * Inserts a portal_channels row for tests. The keypair is generated by
 * the caller (makeRistrettoKeypair) so the test can hold the private key
 * for decrypt roundtrips. Non-key columns are opaque dummy bytes.
 */
export async function seedPortalChannel(
  db: Kysely<TenantDatabase>,
  clientId: ClientId,
  ...
  // care-y-ignore-next-line no-plaintext-db-write -- key material and dummy buffers only, no PII
  const row = await db
