...
describe.skipIf(!process.env.DATABASE_URL)("FollowUpService (DB)", () => {
  ...
  it("listByTicket returns hasRecording true when follow-up has a recording", async () => {
    ...
    await testDb.db
      ...
      .values({
        ticket_id: ticketId,
        followup_id: fu.id,
        blob_key: "blob-vm-test" as BlobKey,
        size_bytes: 1024,
        duration_seconds: 30,
      })
      .execute();
  ...
  it("listByTicket returns hasImage true when follow-up has image attachment", async () => {
    ...
    await testDb.db
      ...
      .values({
        ticket_id: ticketId,
        followup_id: fu.id,
        blob_key: "blob-img-test" as BlobKey,
        size_bytes: 2048,
  ...
  it("listByIds also returns media flags", async () => {
    ...
    await testDb.db
      ...
      .values({
        ticket_id: ticketId,
        followup_id: fu.id,
        blob_key: "blob-file-test" as BlobKey,
        size_bytes: 4096,
  ...
  it("create persists noteTypeId on internal notes", async () => {
    ...
    const noteTypeId = crypto.randomUUID() as NoteTypeId;
    await testDb.db
      .insertInto("note_types")
      .values({
        id: noteTypeId,
  ...
  it("updateInternalNote changes noteTypeId when provided", async () => {
    ...
    const noteTypeA = crypto.randomUUID() as NoteTypeId;
    const noteTypeB = crypto.randomUUID() as NoteTypeId;
    await testDb.db
      .insertInto("note_types")
      .values([
        {
  ...
  it("create persists call metadata fields", async () => {
    ...
    const fu = await svc.create(userId, {
      ...
      isPrivate: false,
      mentionedPseudonyms: [],
      callSid: "CA_test_001" as CallSid,
  ...
  it("listByTicket returns call metadata on phone_call follow-ups", async () => {
    ...
    await svc.create(userId, {
      ...
      isPrivate: false,
      mentionedPseudonyms: [],
      callSid: "CA_list_002" as CallSid,
  ...
  it("listByTicket with mediaFlags=recording returns only follow-ups with recordings", async () => {
    ...
    await testDb.db
      ...
      .values({
        ...
        blob_key: "blob-rec-filter-test" as BlobKey,
        size_bytes: 1024,
        duration_seconds: 15,
      })
      .execute();
  ...
  it("listByTicket with mediaFlags=file returns follow-ups with non-image attachments", async () => {
    ...
    await testDb.db
      ...
      .values({
        ...
        blob_key: "blob-file-filter-test" as BlobKey,
        size_bytes: 2048,
        content_type: "application/pdf",
      })
      .execute();
  ...
  it("listByTicket with mediaFlags=image returns follow-ups with image attachments", async () => {
    ...
    await testDb.db
      ...
      .values({
        ...
        blob_key: "blob-img-filter-test" as BlobKey,
        size_bytes: 4096,
      ...
      .values([
        {
          ticket_id: ticketId,
          followup_id: fu.id,
          blob_key: "blob-rec-1" as BlobKey,
          size_bytes: 1024,
          duration_seconds: 15,
        },
        {
          ticket_id: ticketId,
          followup_id: fu.id,
          blob_key: "blob-rec-2" as BlobKey,
          size_bytes: 2048,
          duration_seconds: 45,
        },
      ])
      ...
      .values([
        {
          ...
          blob_key: "blob-img-summary" as BlobKey,
          size_bytes: 4096,
          content_type: "image/jpeg",
        },
        {
          ...
          blob_key: "blob-file-summary" as BlobKey,
          size_bytes: 8192,
          content_type: "application/pdf",
        },
      ])
  ...
  it("updateInternalNote rejects non-existent follow-up", async () => {
    ...
    await expect(
      svc.updateInternalNote(userId, newFollowupId(), Buffer.from("x")),
    ).rejects.toBeInstanceOf(NotFoundError);
  });
  ...
  it("softDeleteInternalNote rejects non-existent follow-up", async () => {
    ...
    await expect(
      svc.softDeleteInternalNote(userId, newFollowupId(), false),
    ).rejects.toBeInstanceOf(NotFoundError);
  });
  ...
  it("updateOutboundMessage rejects client-sourced messages", async () => {
    ...
    const fuId: FollowupId = newFollowupId();
    await testDb.db
      .insertInto("followups")
      .values({
        id: fuId,
  ...
  it("updateOutboundMessage rejects non-existent follow-up", async () => {
    ...
    await expect(
      svc.updateOutboundMessage(userId, newFollowupId(), Buffer.from("x")),
    ).rejects.toBeInstanceOf(NotFoundError);
  });
