...
const TEST_ORG_ID = orgIdSchema.parse("bcc8e4b4-059b-406c-a455-af40861fbedb");
...
const PHONES = [
  {
    number: e164Schema.parse("+15551111111"),
    sid: phoneSidSchema.parse("PN_hotline"),
  },
  {
    number: e164Schema.parse("+15552222222"),
    sid: phoneSidSchema.parse("PN_system"),
  },
  {
    number: e164Schema.parse("+15553333333"),
    sid: phoneSidSchema.parse("PN_backup"),
  },
] as const;
...
function makeDeps(
  ...
  phones: readonly { number: E164; sid: PhoneSid }[] = PHONES,
): PhoneResolverDeps {
  return {
...
describe("resolveCallerIdByPurpose", () => {
  describe("outbound purpose", () => {
    it("returns number matching phone_outbound_sid", async () => {
      const resolver = createPhoneResolver(
        makeDeps({
          phone_outbound_sid: "PN_hotline" as PhoneSid,
    ...
    it("falls back to first provisioned when SID does not match any number", async () => {
      const resolver = createPhoneResolver(
        makeDeps({
          phone_outbound_sid: "PN_nonexistent" as PhoneSid,
          phone_system_sid: null,
        }),
      );
      const result = await resolver(TEST_ORG, "outbound");
  ...
  describe("system purpose", () => {
    it("returns number matching phone_system_sid", async () => {
      const resolver = createPhoneResolver(
        makeDeps({
          phone_outbound_sid: "PN_hotline" as PhoneSid,
          phone_system_sid: "PN_system" as PhoneSid,
        }),
      );
    ...
    it("falls back through chain when phone_system_sid is stale", async () => {
      const resolver = createPhoneResolver(
        makeDeps({
          phone_outbound_sid: "PN_hotline" as PhoneSid,
          phone_system_sid: "PN_removed" as PhoneSid,
        }),
      );
      const result = await resolver(TEST_ORG, "system");
  ...
  describe("edge cases", () => {
    ...
    it("picks correct number from multiple provisioned", async () => {
      const resolver = createPhoneResolver(
        makeDeps({
          phone_outbound_sid: "PN_backup" as PhoneSid,
          phone_system_sid: null,
        }),
      );
      const result = await resolver(TEST_ORG, "outbound");
