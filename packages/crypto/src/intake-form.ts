 ...
 * form-level metadata.
 *
 * Field labels, configs, and form meta are org-authored content shown on a
 * public page. They are encrypted at rest under the client-branding key
 * (BLAKE2b of the org public key), following the two-tier model: public-page
 * readable, DB dump opaque. Anonymous visitors, volunteers, and admins can
 * all derive this key from the org public key.
 *
...
// Fixed associated data for intake form field ciphertext.
...
export function encryptFieldContent(
  ...
  try {
    const labelBytes = new TextEncoder().encode(JSON.stringify(plain.label));
    ...
    const configBytes = new TextEncoder().encode(JSON.stringify(configPayload));
...
export function decryptFieldContent(
  ...
  try {
    ...
    try {
      const labelJson: unknown = JSON.parse(
        new TextDecoder().decode(labelPlain),
      );
    ...
    try {
      const configJson: unknown = JSON.parse(
        new TextDecoder().decode(configPlain),
      );
...
export function decryptFormMeta(
  ...
  try {
    ...
    try {
      const json: unknown = JSON.parse(new TextDecoder().decode(plain));
