<script lang="ts">
  /**
   * Windowed-mode toolbar above the demo frame.
   *
  ...
  interface Props {
    ...
    onFullscreen: () => void;
    /** When true, the fullscreen button shows minimize instead of expand
     *  and the close button is hidden (used in the drawer footer). */
    exitMode?: boolean;
    /** Pointer handlers wired to App.svelte's drag gesture system.
     *  Omit when docked in a non-draggable context (drawer footer). */
  ...
  //
  // Driven by the bar's rendered width via ResizeObserver. The center
  // zone is absolutely positioned at 50% and can overlap the side zones,
  // so the thresholds account for the sum of all three zones.
  //
  // Full layout: left (136) + center (90) + right badge (~130) + gaps
  //   = ~380px inner. Collapse badge label first, then presets.
  // -----------------------------------------------------------------------

  let barEl: HTMLDivElement | null = $state(null);
  let leftZoneEl: HTMLDivElement | null = $state(null);
  let centerZoneEl: HTMLDivElement | null = $state(null);
  let rightZoneEl: HTMLDivElement | null = $state(null);
  ...
  $effect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      for (const entry of entries) {
        barInnerW = entry.contentRect.width;
      }
    });
    ro.observe(barEl);
  ...
  // Left zone: 3 buttons × 44 + 2 × 2px gap = 136. In exitMode only
  // 1 button = 44.
  const leftW: number = $derived(exitMode ? 44 : 136);

  // Center zone: 2 buttons × 44 + 2px gap = 90 (full), single dropdown
  // = 44 (collapsed), 0 when fully hidden.
  // Right zone: badge ~40px compact, ~130px with label.
  // 16px breathing room between zones.

  const badgeCompact: boolean = $derived(
    shrunk || barInnerW < leftW + 90 + 130 + 16,
  );
  const presetsCollapsed: boolean = $derived(
    shrunk || barInnerW < leftW + 90 + 40 + 16,
  );
  ...
  function handleBarPointerDown(e: PointerEvent): void {
    if (e.button !== 0 || ondragstart == null) return;
    suppressClick = false;
  ...
  function handleBarPointerEnd(e: PointerEvent): void {
    if (pendingPress?.pointerId === e.pointerId) pendingPress = null;
    dragging = false;
    ondragend?.(e);
  }
...
<!-- The bar container is positioned by the parent (App.svelte) via
     .frame-toolbar's absolute placement. The entire background is a
...
<div
  ...
  class:frame-toolbar--docked={exitMode}
...
>
  <!-- Left group: close + minimize/restore + fullscreen (or exit-fullscreen). -->
  <div class="toolbar-zone toolbar-zone--left" bind:this={leftZoneEl}>
    {#if !exitMode}
      <button
        class="toolbar-btn"
        type="button"
    ...
    <button
      class="toolbar-btn"
      class:toolbar-btn--folded={presetsCollapsed && !exitMode}
      type="button"
      onclick={onFullscreen}
      aria-label={exitMode
        ? m.demo_fs_exit()
        : m.demo_toolbar_fullscreen_tooltip()}
      title={exitMode ? m.demo_fs_exit() : m.demo_toolbar_fullscreen_tooltip()}
      aria-hidden={presetsCollapsed && !exitMode}
      tabindex={presetsCollapsed && !exitMode ? -1 : 0}
    >
      {#if exitMode}
        <Minimize2 size={16} />
      {:else}
  ...
  <!-- Right group: role badge (always) + preset picker at compact width. -->
  <div class="toolbar-zone toolbar-zone--right" bind:this={rightZoneEl}>
