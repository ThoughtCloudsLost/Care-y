<script lang="ts">
  ...
  const formQuery = createQuery(() => ({
    queryKey: [...portalKeys.all, "intakeForm"] as const,
    queryFn: async () => {
      if (!trpc.clientPortal) return { formId: null, fields: null };
      return trpc.clientPortal.getIntakeForm.query();
    },
    staleTime: 5 * 60 * 1000,
    retry: false,
  }));
  ...
  const isDefaultForm = $derived(
    resolvedForm.formId === null && !resolvedForm.error,
  );
  ...
  let fieldValues = $state<
    Record<string, string | string[] | AvailabilityData | undefined>
  >({});
  ...
  function validate(): boolean {
    ...
    for (const field of formFields) {
      ...
      if (field.fieldType === "text" || field.fieldType === "textarea") {
        ...
        // field.fieldType === "availability" (only remaining variant)
        if (
          val === undefined ||
          typeof val !== "object" ||
          Array.isArray(val)
  ...
  function focusFirstError(): void {
    if (!browser) return;
    // Focus first field error or contact detail error
    for (const field of formFields) {
  ...
  function handleSubmitError(err: unknown): void {
    ...
    if (code === "TOO_MANY_REQUESTS") {
      // Parse retry-after seconds from the message
    ...
    if (
      ...
      // Re-fetch and re-solve, then retry submission
      powGeneration++;
      submitError = undefined;
      return;
    }
  ...
  function handleFieldChange(
    fieldId: string,
    value: string | string[] | AvailabilityData,
  ): void {
  ...
  {:else}
    ...
    {#each formFields as field (field.id)}
      <IntakeFieldRenderer
        ...
        onchange={(val: string | string[] | AvailabilityData) =>
