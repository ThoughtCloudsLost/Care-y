...
sw.addEventListener("activate", (event: ExtendableEvent) => {
  async function pruneOldCaches(): Promise<void> {
    for (const key of await caches.keys()) {
      if (key !== CACHE_NAME && key !== BRANDING_CACHE_KEY) {
...
sw.addEventListener("fetch", (event: FetchEvent) => {
  ...
  // Dynamic manifest: serve cached org branding in PWA manifest.
  if (url.pathname === "/manifest.webmanifest") {
    event.respondWith(serveDynamicManifest());
    return;
  }
...
/**
 * Serve a dynamic manifest.webmanifest with org branding from the
 * branding cache (if available), otherwise fall back to defaults.
 * Replaces the static manifest generated by @vite-pwa/sveltekit
 * after the SW is installed.
 */
async function serveDynamicManifest(): Promise<Response> {
  ...
  try {
    ...
    const brandingResponse = await cache.match("/branding-data");
    if (brandingResponse) {
