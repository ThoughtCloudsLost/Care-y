<script lang="ts">
  ...
  interface SmsEnrollSheetProps {
    readonly opened: boolean;
    readonly ondismiss: () => void;
    readonly onenrolled: () => void;
  }

  let { opened, ondismiss, onenrolled }: SmsEnrollSheetProps = $props();
  ...
  async function handleSendCode(): Promise<void> {
    ...
    error = "";
    try {
      await trpc.twoFactor.enroll.smsSend.mutate({ phone: phone.trim() });
      codeSent = true;
      startCooldown();
    } catch {
  ...
  async function handleVerify(): Promise<void> {
    ...
    error = "";
    try {
      const result = await trpc.twoFactor.enroll.smsVerify.mutate({ code });
      if (result.success) {
        haptic();
...
<ShellSheet
  {opened}
  {ondismiss}
  ariaLabel={m.twofa_sms_label()}
  title={m.twofa_sms_label()}
>
  {#snippet headerRight()}
    {#if codeSent}
      <SoftButton
  ...
  <div class="sheet-content">
    {#if !codeSent}
      <p class="instruction-text">{m.twofa_sms_desc()}</p>
      <List nested>
        <ListInput
          type="tel"
          label={m.twofa_sms_phone_label()}
          placeholder={m.twofa_sms_phone_placeholder()}
          value={phone}
          oninput={(e: Event) => {
