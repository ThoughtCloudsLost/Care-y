<script lang="ts">
  ...
  interface QueueWatcherPickerProps {
    readonly opened: boolean;
    readonly queueId: string;
    readonly currentWatcherIds: ReadonlySet<string>;
  ...
  const addWatcherMutation = createMutation(() => ({
    mutationFn: async (userId: string) =>
      ticketRouter.addQueueWatcher.mutate({ queueId, userId }),
    onSuccess: () => {
      haptic();
      toastStore.show(m.admin_queue_watcher_added());
      announceToLiveRegion("polite", m.admin_queue_watcher_added());
      void queryClient.invalidateQueries({
        queryKey: queueKeys.watchers(queueId),
      });
    },
    onError: () => {
  ...
  const availableVolunteers = $derived.by((): PickerVolunteer[] => {
    ...
    for (const vol of data) {
      if (currentWatcherIds.has(vol.id)) continue;
...
<ShellSheet
  {opened}
  {ondismiss}
  ariaLabel={m.admin_queue_watcher_picker_title()}
>
  <BlockTitle>{m.admin_queue_watcher_picker_title()}</BlockTitle>

  <div class="picker-search-wrapper">
    <Searchbar
      placeholder={m.admin_queue_watcher_picker_search(withTerms())}
      value={searchValue}
      onInput={(e: Event) => {
  ...
  {:else if availableVolunteers.length === 0}
    <div class="picker-empty" role="status">
      <p>{m.admin_queue_watcher_picker_empty(withTerms())}</p>
    </div>
  {:else}
    <List>
      ...
      {#if filteredVolunteers.length === 0}
        <ListItem title={m.admin_queue_watcher_picker_no_results()} />
      {/if}
    </List>
  {/if}
</ShellSheet>
